...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        //todo: feel free to change code as you need
        Flux<Void> tasks = taskExecutor().flatMap(Flux::from);
    ...
    public void streaming_service() {
        //todo: feel free to change code as you need
        Flux<Message> messageFlux = streamingService().flatMapMany(Flux::from);
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, messageFlux) ->
                signal.get() != null && Objects.equals(signal.get().metaData, "spam")
                ? mailBoxSecondary()
                : messageFlux
        );
        mailBoxPrimary();
    ...
    public void prettify() {
        ...
        openFile().then(writeToFile("0x3522285912341")).then(closeFile()).thenReturn(true);
