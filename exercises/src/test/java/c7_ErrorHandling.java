...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void billion_dollar_mistake() {
        Flux<String> content = getFilesContent()
                ...
                .onErrorContinue((error, file) -> System.out.println("ERROR: " + error.getMessage()))
                ;
    ...
    public void resilience() {
        ...
        Flux<String> content = getFilesContent()
                .flatMap(f -> f.onErrorResume(e -> Mono.empty())); //start from here
    ...
    public void good_old_polling() {
        ...
        Flux<String> alerts = nodeAlerts()
                .repeatWhenEmpty(alert -> alert.delayElements(Duration.ofSeconds(1)))
                .repeat();

//  my solution was:
//                Flux.<Mono<String>>generate(
//                        sink -> sink.next(nodeAlerts())
//                )
//                .flatMap(alert -> alert == null ? Mono.error(new RuntimeException("nothing")) : alert)
